...
interface ButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'danger' | 'ghost' | 'glassy';
  size?: 'xs' | 'sm' | 'md' | 'lg';
  loading?: boolean;
}
...
export function Button({ variant = 'primary', size = 'md', loading, className, children, disabled, ...props }: ButtonProps) {
  ...
  const variants = {
    ...
    glassy: 'backdrop-blur-sm bg-indigo-600/90 text-white border-2 border-white/20 hover:bg-indigo-700 focus:ring-indigo-500 shadow-lg hover:shadow-xl',
  };
  const sizes = {
