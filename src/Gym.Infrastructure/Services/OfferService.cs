...
public class OfferService : IOfferService
{
    ...
    public OfferService(IRepository<Offer> offerRepository, IUnitOfWork unitOfWork, IMapper mapper, IStringLocalizer<ApplicationResources> localizer)
    {
    ...
    public async Task<Result<OfferDto>> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        if (offer is null)
            return Result<OfferDto>.Failure(_localizer["Offer not found"]);
    ...
    public async Task<Result<Guid>> CreateAsync(CreateOfferDto dto, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(dto.OfferTitle))
            errors.Add(_localizer["Offer title is required"]);

        if (dto.EndDate <= dto.StartDate)
            errors.Add(_localizer["End date must be after start date"]);

        if (dto.OfferPrice == null || dto.OfferPrice <= 0)
            errors.Add(_localizer["Offer price is required and must be positive"]);
    ...
    public async Task<Result> ToggleActiveAsync(Guid id, bool isActive, CancellationToken cancellationToken = default)
    {
        ...
        return Result.Success(isActive ? _localizer["Offer activated"] : _localizer["Offer deactivated"]);
    }
        ...
        int? packageDurationMonths, CancellationToken cancellationToken = default)
    {
        ...
        if (offer is null)
            return Result<AppliedOfferDto>.Failure(_localizer["Offer not found"]);

        if (!offer.IsActive || offer.EndDate < DateTime.UtcNow)
            return Result<AppliedOfferDto>.Failure(_localizer["Offer has expired and cannot be applied"]);
        ...
        if (offer.OfferType == OfferType.BonusDuration)
        {
            ...
            result.Description = string.Format(_localizer["Duration: {0} months ({1} base + {2} bonus)"], result.FinalDurationMonths, packageDurationMonths ?? 0, offer.BonusMonths ?? 0);
        }
        else if (offer.OfferType == OfferType.FixedPrice)
        {
            ...
            result.Description = string.Format(_localizer["Fixed price: {0:N2} EGP (original: {1:N2} EGP)"], offer.OfferPrice ?? 0, packagePrice ?? 0);
        }
        else if (offer.OfferType == OfferType.ExtraFreeze)
        {
            ...
            result.Description = string.Format(_localizer["Extra freeze days: +{0} days"], offer.ExtraFreezeDays);
        }
        else if (offer.OfferType == OfferType.FreeRegistration)
        {
            result.FinalDurationMonths = packageDurationMonths ?? 0;
            result.FinalPrice = 0;
            result.Description = _localizer["Free registration - price set to 0"];
        }
        else
        {
            result.FinalDurationMonths = packageDurationMonths ?? 0;
            result.FinalPrice = packagePrice ?? 0;
            result.Description = offer.Description ?? _localizer["Custom offer applied"];
        }
